...
target_word_f <- "service"
target_word_s <- "experience"
cor_limit <- 0.4


# Oblicz asocjacje dla tego słowa
associations_f <- findAssocs(tdm, target_word_f, corlimit = cor_limit)
assoc_vector_f <- associations_f[[target_word_f]]
assoc_sorted_f <- sort(assoc_vector_f, decreasing = TRUE)


# Ramka danych
assoc_df_f <- data.frame(
  word = factor(names(assoc_sorted_f), levels = names(assoc_sorted_f)[order(assoc_sorted_f)]),
  score = assoc_sorted_f
)

# Oblicz asocjacje dla tego słowa
associations_s <- findAssocs(tdm, target_word_s, corlimit = cor_limit)
assoc_vector_s <- associations_s[[target_word_s]]
assoc_sorted_s <- sort(assoc_vector_s, decreasing = TRUE)


# Ramka danych
assoc_df_s <- data.frame(
  word = factor(names(assoc_sorted_s), levels = names(assoc_sorted_s)[order(assoc_sorted_s)]),
  score = assoc_sorted_s
)


# Wykres lizakowy (lollipop chart)
# stosowany w raportach biznesowych i dashboardach:
ggplot(assoc_df_f, aes(x = score, y = reorder(word, score))) +
  ...
  scale_x_continuous(limits = c(0, max(assoc_df_f$score) + 0.1), expand = expansion(mult = c(0, 0.2))) +
  theme_minimal(base_size = 12) +
  labs(
    title = paste0("Asocjacje z terminem: '", target_word_f, "'"),
...
# Wykres lizakowy (lollipop chart)
# stosowany w raportach biznesowych i dashboardach:
ggplot(assoc_df_s, aes(x = score, y = reorder(word, score))) +
  geom_segment(aes(x = 0, xend = score, y = word, yend = word), color = "#a6bddb", size = 1.2) +
  geom_point(color = "#0570b0", size = 4) +
  geom_text(aes(label = round(score, 2)), hjust = -0.3, size = 3.5, color = "black") +
  scale_x_continuous(limits = c(0, max(assoc_df_s$score) + 0.1), expand = expansion(mult = c(0, 0.2))) +
  theme_minimal(base_size = 12) +
  labs(
    title = paste0("Asocjacje z terminem: '", target_word_s, "'"),
    subtitle = paste0("Próg r ≥ ", cor_limit),
    x = "Współczynnik korelacji Pearsona",
    y = "Słowo"
  ) +
...
# Wykres lizakowy z natężeniem
# na podstawie wartości korelacji score:
ggplot(assoc_df_s, aes(x = score, y = reorder(word, score), color = score)) +
  geom_segment(aes(x = 0, xend = score, y = word, yend = word), size = 1.2) +
  geom_point(size = 4) +
  geom_text(aes(label = round(score, 2)), hjust = -0.3, size = 3.5, color = "black") +
  scale_color_gradient(low = "#a6bddb", high = "#08306b") +
  scale_x_continuous(
    limits = c(0, max(assoc_df_s$score) + 0.1),
    expand = expansion(mult = c(0, 0.2))
  ) +
  theme_minimal(base_size = 12) +
  labs(
